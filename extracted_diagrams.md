...
Doctor Portal: An e-commerce-style interface where doctors can register, browse a categorised medicine catalogue with images and pricing, add items to a shopping cart with real-time tax computation, checkout using Online/COD/Credit payment, and track orders through completion. Doctors can view and download GST-compliant invoices for every order.
Admin Portal: A comprehensive management dashboard where distributors can manage their medicine inventory (add, edit, delete products with images), process incoming orders (approve, dispatch, complete), record wholesale purchase entries from manufacturers (with HSN codes, batch numbers, scheme/discount percentages), reconcile payments, and view business analytics.
3.6 Conceptual Models
3.6.1 Doctor Ordering Flow
Doctor registers/logs in → receives JWT token
browses medicine catalogue (filtered by search/category)
adds items to cart → CartContext computes real-time GST
proceeds to checkout → selects payment mode
(if Online) → Razorpay popup → payment verification
Backend validates stock → atomically deducts inventory
Order created → Invoice auto-generated → confirmation displayed
Doctor can track order status and view/download invoice
3.6.2 Admin Inventory Flow
Admin logs in → JWT token with admin role
Records purchase entry from manufacturer → items with batch, HSN, qty
System automatically increments stock in Medicine collection
Admin receives new orders → reviews and updates status
Admin can record payment entries against orders
Dashboard reflects updated analytics (revenue, stock alerts)
